...
Run the Next.js frontend and Express backend concurrently:
...
> **Note:** Puppeteer requires Chromium to be available on your server. On most Linux environments (e.g., AWS EC2, Railway, Render), you may need to install additional system dependencies. See the [Puppeteer troubleshooting guide](https://pptr.dev/troubleshooting) for details.
...
All API endpoints are served from the Express backend (`NEXT_PUBLIC_API_URL`).
...
```json
{
  ...
  "rewrittenBullets": [
    {
      "original": "Worked on API performance.",
      "rewritten": "Reduced API P99 latency by 38% by migrating to Redis caching, serving 2M+ daily requests."
    }
  ],
  "highlightedTerms": ["Redis", "Node.js", "REST API"]
}
```
...
Generate a PDF from resume JSON and layout settings.
...
Get job role recommendations and magic links.
...
```bash
git commit -m "feat: add support for dark/light theme toggle"
```

Follow [Conventional Commits](https://www.conventionalcommits.org/) where possible (`feat:`, `fix:`, `docs:`, `refactor:`).
...
Open a PR against the `main` branch and fill out the PR template.

### Reporting Issues

Please use the [GitHub Issues](https://github.com/your-username/elevate/issues) page. Include:
...
- The Gemini prompt templates live in `lib/gemini.js` — this is the best place to improve AI output quality.
- PDF layout logic is in `server/routes/pdf.js`. The HTML template used by Puppeteer is in `templates/resume.html`.
- All UI components use Tailwind CSS v4 utility classes — avoid inline styles.
